...
function GoogleLoginPanel() {
  ...
  const googleLogin = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      ...
      try {
        const data = await requestContract('authGoogle', undefined, {
          body: {
  ...
  return (
    <div className={styles.page}>
      <FormContainer maxWidth={440}>
        {!googleClientId ? (
          <>
            <FormTitle>Login unavailable</FormTitle>
            <p className={styles.subtitle}>Google OAuth is not configured for this environment yet.</p>
          </>
        ) : (
          <GoogleLoginPanel />
        )}
