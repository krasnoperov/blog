...
I lived with that pattern for maybe a month before I had to do something about it. By that point I was hand-reviewing every PR for "is anything else in the repo stale because of this change," which was exactly the bottleneck I had been running an agent to avoid.
...
The first weeks of running review-quill against patchrelay-produced PRs looked broken.

The pattern was: agent pushes a branch, review-quill rejects, agent pushes a fix, review-quill rejects, agent pushes again, review-quill finally approves. Three to five rounds of back-and-forth was normal. Sometimes more. I have issues in the database where review-fix ran twelve, fourteen, twenty-five times before the PR finally got through.

The shape looked like a stuck loop. Two LLMs talking past each other.

What I expected to find when I dug in was that review-quill was being unreasonable — reviewer hallucination, bikeshedding, latching onto an early objection it then could not let go of. There is some of that. There is a documented incident on a subtitles PR where review-quill flipped its own stance across three consecutive rounds, which is the failure mode in its purest form.
...
If a human reviewer were on the other end of this loop, "twenty-five rounds before approval" would be a process failure. The reviewer would burn out, the implementer would burn out, the team would shorten the review to make it survivable, and standards would slip. That's not hypothetical — it's how most code review actually erodes.

A coding agent doesn't burn out. Neither does the reviewer when the reviewer is also a service. The hand-off between review and fix takes seconds, not days. There's no Slack DM, no "can we sync about this," no waiting for a context switch. The agent reads the structured review on a `REQUEST_CHANGES` exit, opens the worktree it's been working in, addresses the comments, pushes, and the loop runs again.
...
The honest caveat: the same fast hand-off that makes the strict-review loop affordable also makes it fail in a particular way. Sometimes the two agents settle into churn — review-quill names a symptom, the implementer fixes that symptom, review-quill finds the next symptom of the same underlying issue, the implementer patches that, and neither side ever steps back to ask what's actually wrong. The PR converges in the small and stays broken in the large.

The current mitigation is crude. Patchrelay caps the number of `review_fix` rounds for any single PR at a hard limit, after which the issue escalates for human attention instead of letting the loop spend my budget on surface fixes. The cap stops the bleeding. It does not address the cause.
...
If the reviewer is mostly right and the implementation is mostly close-but-wrong, the question shifts. It is no longer "how do I tune the reviewer." It is "how do I get the implementer to land closer to right on the first attempt, and to address feedback meaningfully when it doesn't."
