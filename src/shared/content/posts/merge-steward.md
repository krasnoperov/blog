...
The win from going to a real merge queue, even a simple one, was bigger than I expected. Most of the day-to-day failures I used to handle by hand — rebases, speculative re-fetches, "flake or real fail?" branching — are now handled by the steward as a side effect of being a queue at all.
...
The model is bad at deterministic queue control. It doesn't keep a stable picture of "what just changed and what should happen next" across a batch of PRs. It works one run at a time, from whatever context I hand it. A merge queue is the opposite: the global state — what's validating now, what's queued behind it, what just landed — matters more than any single decision. Asking the model to make those decisions one at a time was burning tokens, time, and my own attention on something that wanted to be a small finite-state machine.
